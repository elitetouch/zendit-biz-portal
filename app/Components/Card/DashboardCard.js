...
function DashboardCard({title, routeFunc,formPage,storeTracker,icon, Total_number, bgColor, textColor,  partners}) {
  return (
     ...
     className='   w-full h-[150px] rounded-lg grid items-center bg-white '>
        <Box className=' w-11/12 m-auto pt-[5px]'>
            <Box className=' mt-[10px] flex items-center justify-between'>
                ...
                color={textColor}
                className=' text-[15px] font-semibold'>{title}</Text>
                 <Box backgroundColor={bgColor} className={`bg-[${bgColor}] rounded-lg h-[38px] w-[38px] grid justify-center items-center  `}>
                     {icon}
                 </Box>
            </Box>        
          ...
          {Total_number}
               ...
               { !partners && <Text className=' text-gray-500 text-[15px] pt-[3px]'>Parcels</Text>}
               </Box>
            </Box>
        </Box>
    </Box>
