...
function ChakraInputs({
  ...
  return (
    <Box>
      <Text
        className={` mt-[20px] font-semibold ${whiteBorder && "text-white"}`}
      >
        {label}
      </Text>
      <Box
        ...
        border="1px"
        borderColor={whiteBorder ? "white" : "gray.300"}
        borderRadius="lg"
        className=" w-full grid items-center lg:mt-[15px] mt-[10px] "
      >
        <Box className="flex items-center gap-x-[10px] w-11/12 m-auto">
          ...
          <Input
            ...
            border={"none"}
            className={`${
              whiteBorder ? "text-white" : "text-[#7C7C7C]"
            } text-[14px] mr-[10px]`}
            placeholder={placing}
            _autofill={false}
            autoComplete="off"
