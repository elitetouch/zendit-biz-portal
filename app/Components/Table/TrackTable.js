...
function TrackTable({setDisplayBtn}) {
  ...
  const column=[
          {
        name:'Date Picked',selector:row =><Text className='text-[12px]'>{row.date} {row.time}</Text>,
         wrap:true
      },
      {
        name:'Client',selector:row =><Text className='text-[12px]'>{row.Item}</Text>,
         wrap:true
      },

     
      {
        name:'location',selector:row =><Text className='text-[12px]'>{row.Pick_Up_Location}</Text>,
         wrap:true
      },
       {
        name:'Partners',selector:row =><Text className='text-[12px]'>{row.Client}</Text>,
         wrap:true
      },
      {
        name:'Receiver',selector:row =><Box><Text className='text-[12px]'><span className=' font-semibold'>Name:</span>{row.Recievers_Name}</Text>
       ...
       {
        name:'Rider details',selector:row =><Text className='text-[12px]'>{row.Client}</Text>,
         wrap:true
      },
      {
                 name:'Status',selector:row =><Box
                         roundedLeft={'full'}
                               roundedRight={'full'}
                               backgroundColor={row.Status==='Paid'&&'#DEEBFC'||row.Status==='Unpaid'&&'#FFF2D1'}
                         className= {`h-[32px] w-[94px] grid justify-center items-center ${row.status==='online'?'bg-[#1EBD5D]':'bg-[#DEEBFC]'}`} >
                           <Text color={row.Status==='Paid'&&'#398EF1'||row.Status==='Unpaid'&&'#FE9534'||row.Status==='Ongoing'&&'gray'||row.Status==='Delivered'&&'green'} className='text-[12px] font-semibold'>{row.Status}</Text>
